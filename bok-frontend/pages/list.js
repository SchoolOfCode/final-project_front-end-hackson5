...
const urlQuery = id.split("/")
  ...
  //Function that edits the name of a list
  const editListName = async () => {
    ...
    .then(async ()=>{
      ...
      const response = await fetch(`https://hackson5.herokuapp.com/readinglist/${urlQuery[0]}/${urlQuery[1]}`)
      const data = await response.json() 
      //console.log(data.payload[0].reading_list_name)
      setUniqueListId(data.payload[0].reading_list_name)
      router.push(`list/?id=${urlQuery[0]}/${urlQuery[1]}/${data.payload[0].reading_list_name}`, undefined, {shallow: true})
    }
    )
  }

//const [uniqueListId, setUniqueListId] = useState("")
